...
private object Utils {
  def start(move: String): Position = {
    rejectInvalidMove(move)
    new Position(move.substring(0, 2))
  }

  def end(move: String): Position = {
    rejectInvalidMove(move)
    new Position(move.substring(2, 4))
  }

  private def rejectInvalidMove(move: String): Unit =
    require(move.length == 4, "Move must have length 4: " + move)
