package chess.model

import chess.model.Colour.Black
import chess.test.TestUtils
import org.scalatest.*
import matchers.should.Matchers
import wordspec.AnyWordSpec

class DelegatingConfigurationViewTest extends AnyWordSpec with Matchers with TestUtils {

  "A DelegatingConfigurationView" should {
    "delegate" in {
      val conf = new GridConfiguration
      conf.add("e5", Black, Pawn)
      val del = new DelegatingConfigurationView(conf).applied(MovePiece("e5", "e6"))
      del.getExistingPiece("e6") shouldBe Placed(Black, Pawn, Some(new Position("e5")))
    }
  }
}
